...
const res = await fetch(`${API_URL}/api/emprestimos/estatisticas`);
 ...
 const buscarTodos = async () => {
              ...
              <>
                ...
                {estatisticas && (
                  <Grid container spacing={3} sx={{ mb: 3 }}>
                    <Grid item xs={12} sm={4}>
                      <Card sx={{ height: 120, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
                        <CardContent sx={{ textAlign: 'center' }}>
                          <Typography variant="h6" color="primary" gutterBottom>
                            <AssessmentIcon sx={{ verticalAlign: 'middle', mr: 1 }} />
                            Total de clientes
                          </Typography>
                          <Typography variant="h3">{estatClientes.total}</Typography>
                        </CardContent>
                      </Card>
                    </Grid>
                    <Grid item xs={12} sm={4}>
                      <Card sx={{ height: 120, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
                        <CardContent sx={{ textAlign: 'center' }}>
                          ...
                          {(estatClientes.porStatus || []).map(s => (
                            <Typography key={s.status} variant="body1">
                    ...
                    <Grid item xs={12} sm={4}>
                      <Card sx={{ height: 120, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
                        <CardContent sx={{ textAlign: 'center' }}>
                          ...
                          {(estatFluxo.porFluxo && estatFluxo.porFluxo.length > 0) ? (
                            estatFluxo.porFluxo.map(f => (
                              <Typography key={f.status_fluxo} variant="body1">
                                {f.status_fluxo}: <b>{f.count}</b>
                              </Typography>
                            ))
                          ) : (
                ...
                <Grid container spacing={3} sx={{ mb: 3 }}>
                  <Grid item xs={12} sm={2}>
                    <Card sx={{ minHeight: 100, background: "#e3f6e8" }}>
                      <CardContent sx={{ textAlign: 'center' }}>
                  ...
                  <Grid item xs={12} sm={2}>
                    <Card sx={{ minHeight: 100, background: "#f0e6fa" }}>
                      <CardContent sx={{ textAlign: 'center' }}>
                        <Typography variant="subtitle2" color="primary" gutterBottom>
                  ...
                  <Grid item xs={12} sm={2}>
                    <Card sx={{ minHeight: 100, background: "#fffbe6" }}>
                      <CardContent sx={{ textAlign: 'center' }}>
                        <Typography variant="subtitle2" color="primary" gutterBottom>
                          Juros pagos (30%)
                        </Typography>
                  ...
                  <Grid item xs={12} sm={2}>
                    <Card sx={{ minHeight: 100, background: "#e3eaf6" }}>
                      <CardContent sx={{ textAlign: 'center' }}>
                        <Typography variant="subtitle2" color="primary" gutterBottom>
                  ...
                  <Grid item xs={12} sm={2}>
                    <Card sx={{ minHeight: 100, background: "#ffeaea" }}>
                      <CardContent sx={{ textAlign: 'center' }}>
                ...
                <Paper elevation={2} sx={{ p: 3, mb: 3 }}>
                  <form onSubmit={handleBuscar} style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                    <TextField
                      ...
                      sx={{ flex: 1 }}
                    />
                    <Button type="submit" variant="contained" color="primary" sx={{ height: 40, minWidth: 100 }}>
                      BUSCAR
                    </Button>
                    <Button
                      type="button"
                ...
                {mostrarLista && listaCadastros.length > 0 && (
                  <div style={{ marginBottom: 32 }}>
                    <Typography variant="h6" sx={{ mb: 1 }}>Todos os Cadastros</Typography>
                      ...
                      <Table>
                        ...
                        <TableBody>
                          {listaCadastros
                            ...
                            .map(cad => {
                              // Busca o último empréstimo desse CPF
                              const emprestimoCliente = emprestimos.filter(emp => emp.cpf === cad.cpf);
                              let ultimoEmprestimo = null;
                              if (emprestimoCliente.length > 0) {
                                ultimoEmprestimo = [...emprestimoCliente].sort((a, b) => {
  ...
  sx={{
    mb: 2,
    background:
      cadastro.status === 'Aprovado' ? '#e3f6e8'
      : cadastro.status === 'Rejeitado' ? '#ffeaea'
      : cadastro.status === 'Em análise' ? '#fffbe6'
      : '#f5f5f5'
  }}
>
  <CardContent>
    <Typography variant="h6" color="primary" gutterBottom>Status do Cadastro</Typography>
    <Typography sx={{ mb: 1 }}>
      <b>Status atual:</b>{' '}
      <span style={{
        color: cadastro.status === 'Aprovado' ? 'green' :
              cadastro.status === 'Rejeitado' ? 'red' : 'orange',
        fontWeight: 'bold'
      }}>
        {cadastro.status}
      </span>
    ...
    exclusive
    onChange={(_, novoStatus) => {
      if (!novoStatus) return;
      if (novoStatus === 'Aprovado') {
        setMostrarDataAprovacao(true);
      } else {
  ...
  >
    <ToggleButton
      ...
      sx={{
        color: '#ff9800',
        fontWeight: 'bold',
        '&.Mui-selected': { background: '#ffe0b2', color: '#ff9800' }
      }}
    >
      Em análise
    </ToggleButton>
    <ToggleButton
      ...
      sx={{
        color: '#388e3c',
        fontWeight: 'bold',
        '&.Mui-selected': { background: '#c8e6c9', color: '#388e3c' }
      }}
    >
      Aprovado
    </ToggleButton>
    <ToggleButton
      ...
      sx={{
        color: '#d32f2f',
        fontWeight: 'bold',
        '&.Mui-selected': { background: '#ffcdd2', color: '#d32f2f' }
      }}
    >
      Rejeitado
    </ToggleButton>
...
if (emprestimoInicial) {
  ...
  await fetch(`${API_URL}/api/emprestimos/${emprestimoInicial.id}/fluxo`, {
    ...
    body: JSON.stringify({
      status_fluxo: 'Em dia',
      data_emprestimo: dataAprovacao,
      data_pagamento: dataPagamento
    })
  });
}

setMostrarDataAprovacao(false);
setDataAprovacao('');
await buscarDetalhes(cadastro.cpf);
await buscarTodos();
          }}
        ...
        >
          Cancelar
                ...
                {/* Fluxo do Empréstimo */}
                {cadastro.status === 'Aprovado' && emprestimoAtivo && (
                ...
                {/* Blocos de dados pessoais, profissionais, etc... */}
                <Card sx={{ mb: 2 }}>
                  <CardContent>
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      if (window.confirm('Tem certeza que deseja excluir esta ficha? Esta ação não poderá ser desfeita.')) {
                        ...
                        setCadastro(null);
                        setEmprestimoAtivo(null);
                        setMensagem('Ficha excluída com sucesso.');
                        buscarTodos();
                        buscarEstatisticas();
                      }
                    }}
