...
export default function Atrasados({ onVoltar }) {
  ...
  const marcarComoQuitado = async (id) => {
    try {
      ...
      setAtrasados(atrasados => atrasados.filter(e => e.id !== id));
    } catch (err) {
  ...
  const marcarPagamento30 = async (item) => {
    try {
      ...
      setAtrasados(atrasados => atrasados.filter(e => e.id !== item.id));
    } catch (err) {
  ...
  const marcarAcao = (acao) => {
    const item = atrasados.find(e => e.id === acaoId);
    if (!item) return;
  ...
  return (
    <Box sx={{ maxWidth: 1200, mx: "auto", mt: 2 }}>
      ...
      {carregando ? (
        ...
        <Typography>Nenhum cliente em atraso.</Typography>
      ) : (
              ...
              <Grid item xs={12} md={6} key={item.id}>
                <Card sx={{ borderRadius: 3, boxShadow: 3, bgcolor: "#fffbea" }}>
                  <CardContent>
                    ...
                    <Box sx={{ mb: 1 }}>
                      <Chip label="Em atraso" color="error" sx={{ fontWeight: "bold" }} />
                    </Box>
